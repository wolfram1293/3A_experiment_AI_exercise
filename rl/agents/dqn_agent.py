...
from agents.models.dqn_model import DQNModel as DQNMC
...
class DQNAgent(abstract_agent.Agent):
    ...
    def __init__(self, env):
        ...
        self.policy_net = DQNMC(self.obs_size, self.n_actions).to(self.device)
        self.target_net = DQNMC(self.obs_size, self.n_actions).to(self.device)
    ...
    def select_action(self, obs, explore):
        ...
        if explore:
            ...
            eps = self.eps_end + (self.eps_start - self.eps_end) * np.exp(-self.timestep / self.eps_decay)
            if sample > eps:
                with torch.no_grad():
    ...
    def train(self, obs, reward, done):
        ...
        if self.replay_start_size >= len(self.replay_buffer):
            ...
            with torch.no_grad():
                batch_next_action = self.policy_net(batch_next_state).max(1)[1].view(-1, 1)
