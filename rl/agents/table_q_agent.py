...
class TableQAgent(abstract_agent.Agent):
    ...
    def act(self, obs):
        return self.select_action2(obs)
    ...
    def train(self, obs, reward):
        if self.last_obs is not None:
            ...
            if obs_key in self.q_table:
                ...
                # ------------
                max_q = np.max(self.q_table[obs_key][:self.action_num])
    ...
    def epsilon_greedy(self, obs_key):
        ...
        # ------------
        max_q_action = np.argmax(self.q_table[obs_key][:self.action_num])
        #raise NotImplementedError()
        # ------------
        ...
        # ------------
        action = np.random.choice(a = [random_action, max_q_action], p = [self.exploration_prob, 1-self.exploration_prob])
        # raise NotImplementedError()
        # ------------
    ...
    def greedy(self, obs_key):
        action = np.argmax(self.q_table[obs_key][:self.action_num])
        return action
